...
public class ModificarUsuarioAnalista extends JPanel {
	...
	private JLabel lblEstadoActivacion;
	...
	private JRadioButton rdbtnSinValidar;
	private JRadioButton rdbtnValidado;
	private JRadioButton rdbtnEliminado;
	private final ButtonGroup buttonGroup = new ButtonGroup();
	
	private static ModificarUsuarioAnalista vista = new ModificarUsuarioAnalista();
	
	/**
	 * Create the panel.
	 */
	public ModificarUsuarioAnalista() {
		...
		lblArea.setBounds(403, 230, 135, 21);
		add(lblArea);
		...
		lblTipoUsuario.setBounds(403, 175, 135, 21);
		add(lblTipoUsuario);
		...
		txtGeneracion.setBounds(566, 195, 140, 20);
		add(txtGeneracion);
		...
		spRol.setBounds(566, 270, 140, 14);
		add(spRol);
		...
		cmbArea.setBounds(403, 250, 140, 20);
		add(cmbArea);
		...
		cmbRol.setBounds(566, 250, 140, 20);
		add(cmbRol);
		...
		cmbTipoUsuario.setBounds(403, 195, 140, 20);
		add(cmbTipoUsuario);
		
		btnRegistrar = new JButton("CONFIRMAR CAMBIOS");
		btnRegistrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {				
				validarCamposIngresados();
				
				if (rdbtnEliminado.isSelected()) {
					int respEliminado = JOptionPane.showConfirmDialog(null, 
							"El usuario se eliminará de forma lógica. ¿Confirma?", "ATENCIÓN", JOptionPane.YES_NO_OPTION);
					System.out.println(respEliminado);
				} else {
					int respModificado = JOptionPane.showConfirmDialog(null, 
							"El usuario será modificado. ¿Confirma?", "ATENCIÓN", JOptionPane.YES_NO_OPTION);
					System.out.println(respModificado);
				}
			}
		});
		...
		btnRegistrar.setBounds(443, 368, 223, 49);
		add(btnRegistrar);
		
		lblModificarUsuario = new JLabel("MODIFICAR USUARIO");
		lblModificarUsuario.setForeground(Color.BLACK);
		lblModificarUsuario.setFont(new Font("Segoe UI", Font.BOLD, 20));
		lblModificarUsuario.setBounds(40, 12, 390, 41);
		add(lblModificarUsuario);
		
		
		lblAviso = new JLabel("Los campos marcados con * son obligatorios.");
		lblAviso.setForeground(Color.DARK_GRAY);
		lblAviso.setFont(new Font("Segoe UI", Font.ITALIC, 10));
		lblAviso.setBounds(40, 404, 230, 13);
		add(lblAviso);
		
		rdbtnSinValidar = new JRadioButton("Sin validar");
		buttonGroup.add(rdbtnSinValidar);
		rdbtnSinValidar.setBackground(Color.WHITE);
		rdbtnSinValidar.setFont(new Font("Segoe UI Light", Font.PLAIN, 14));
		rdbtnSinValidar.setBounds(403, 305, 96, 21);
		add(rdbtnSinValidar);
		
		rdbtnValidado = new JRadioButton("Validado");
		buttonGroup.add(rdbtnValidado);
		rdbtnValidado.setBackground(Color.WHITE);
		rdbtnValidado.setFont(new Font("Segoe UI Light", Font.PLAIN, 14));
		rdbtnValidado.setBounds(508, 305, 90, 21);
		add(rdbtnValidado);
		
		rdbtnEliminado = new JRadioButton("Eliminado");
		buttonGroup.add(rdbtnEliminado);
		rdbtnEliminado.setBackground(Color.WHITE);
		rdbtnEliminado.setFont(new Font("Segoe UI Light", Font.PLAIN, 14));
		rdbtnEliminado.setBounds(600, 305, 103, 21);
		add(rdbtnEliminado);
		
		lblEstadoActivacion = new JLabel("Estado");
		lblEstadoActivacion.setFont(new Font("Segoe UI", Font.PLAIN, 13));
		lblEstadoActivacion.setBounds(403, 285, 45, 13);
		add(lblEstadoActivacion);
		
	}
	
	public static ModificarUsuarioAnalista getVista() {
		return vista;
	}
	...
	public void validarCamposIngresados() {
		...
		//Verifico que la CÉDULA tenga el formato correcto
			...
			JOptionPane.showMessageDialog(null, 
					"El documento que ingresó es incorrecto", "ATENCIÓN", JOptionPane.WARNING_MESSAGE);
		} else {
			JOptionPane.showMessageDialog(null, 
